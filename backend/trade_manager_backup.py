...
class TradeManager:
    def __init__(self, api, cfg, websocket_manager=None):
        self.api = api
    ...
    def on_fill_notification(self, stock_code: str, fill_price: float, quantity: int):
        self.logger.info(f"✅ 체결 알림: {stock_code} {quantity}주 @ {fill_price}원")
        self.handle_stoploss_register(stock_code, fill_price, quantity)

    def handle_signing_notice(self, notice_data):
        stock_code = notice_data.get("stock_code")
        exec_qty = notice_data.get("exec_qty", 0)
        exec_price = notice_data.get("exec_price", 0)
        if exec_qty >= 1 and notice_data.get("order_type_detail") == "매수":
            logger.info(f"[{stock_code}] 실시간 체결 통보 수신됨. {exec_qty}주 @ {exec_price}")
            atr = calculate_atr(stock_code, period=20, api_instance=self.api, return_only=True)
            if atr:
                self.setup_stoploss(stock_code, exec_price, atr, exec_qty)
    ...
    def monitor_order_fill(self):
        def run():
            while True:
                ...
                logger.info(f"🟢 체결 확인 중 - 주문번호: {order_id}, 종목코드: {stock_code}")

                try:
                    message = self.websocket_manager.wait_for_fill(order_id, timeout=60)
                    if message:
                        logger.info(f"✅ 체결 감지됨 - 주문번호: {order_id}, 종목코드: {stock_code}, 메시지: {message}")
                        self.save_trade_log(order, message)
                    else:
                        logger.warning(f"⚠️ 체결 실패 또는 타임아웃 - 주문번호: {order_id}")
                except Exception as e:
                    logger.error(f"❌ 체결 확인 중 오류 - 주문번호: {order_id}, 에러: {e}")
    ...
    def register_order_fill_listener(self, stock_code, order_id):
        """
        주문 체결 콜백을 등록합니다.
        """
        def on_fill_callback(filled_qty, fill_price):
            logger.info(f"📬 체결콜백 발동됨 - 종목코드: {stock_code}, 주문번호: {order_id}, 체결수량: {filled_qty}")
            # 실제 체결 처리 로직을 여기에 작성
            # 예: self.on_fill_notification(stock_code, fill_price, filled_qty)
            pass

        # 예시: self.api.register_fill_callback(order_id, on_fill_callback)
        self.api.register_fill_callback(order_id, on_fill_callback)
        logger.info(f"🧲 체결통보 콜백 등록됨 - 종목코드: {stock_code}, 주문번호: {order_id}")

    # 🧩 체결통보 메시지 처리 (queue 수신)
    def process_execution_queue(self):
        while True:
            try:
                message = self.execution_queue.get(timeout=1)
                logger.info(f"📥 체결통보 수신됨 (트레이드매니저): {message}")
            except queue.Empty:
                continue
