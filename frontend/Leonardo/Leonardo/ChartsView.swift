        ...
        HStack(alignment: .top) {
            ...
            if !priceInfo.isEmpty {
                VStack(alignment: .leading) {
                    Text("총자산: \(formattedNumber("\(appModel.totalAsset)"))")
                    Text("ATR기간: \(appModel.atrPeriod)일")
                    Text("손실허용비율: \((riskRatio * 100).formatted(.number.precision(.fractionLength(1))))%")
                    Text("ATR값: \(formattedNumber("\(Int(atrValue.rounded(.down)))"))")
                    Text("매수량: \(calculatedQuantity)주")
                    Text("목표매수가: \(formattedNumber("\(Int(entryPrice))"))")
                    Text("손절가: \(formattedNumber("\(Int(stopLoss))"))")
                    Text("투입자금: \(formattedNumber("\(Int(totalInvestment))"))")
                    Text("투입비중: \((investmentRatio * 100).formatted(.number.precision(.fractionLength(1))))%")
                }
                .padding(.horizontal)
                .foregroundColor(.secondary)
            }
        ...
        .alert("매수를 하시겠습니까?", isPresented: $showBuyConfirmation) {
            ...
            if let info = pendingBuyInfo {
                ...
                Text("""
                    ...
                    종목코드: \(stock.Code)
                    주문수량: \(info.quantity)
                    목표매수가: \(formattedNumber("\(adjustToHoga(price: info.entryPrice))"))
